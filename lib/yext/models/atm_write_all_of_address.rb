...
module Yext
  # Contains the address of the entity (or where the entity is located)  Must be a valid address Cannot be a P.O. Box  If the entity is an `event`, either an **`address`** value or a **`linkedLocation`** value can be provided.
  class AtmWriteAllOfAddress
    # The city the entity (or the entity's location) is in   Cannot Include: * a URL or domain name * a digit
    attr_accessor :city

    attr_accessor :country_code

    # Provides additional information to help consumers get to the entity. This string appears along with the entity's address (e.g., `In Menlo Mall, 3rd Floor`). It may also be used in conjunction with a hidden address (i.e., when **`addressHidden`** is `true`) to give consumers information about where the entity can be found (e.g., `Servicing the New York area`).
    attr_accessor :extra_description

    #  Cannot Include: * a URL or domain name
    attr_accessor :line1
    ...
    # The entity's postal code. The postal code must be valid for the entity's country. Cannot include a URL or domain name.   Cannot Include: * a URL or domain name
    attr_accessor :postal_code

    # The name of the entity's region or state.   Cannot Include: * a URL or domain name
    attr_accessor :region

    # The name of the entity's sublocality   Cannot Include: * a URL or domain name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yext::AtmWriteAllOfAddress` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yext::AtmWriteAllOfAddress`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
