  ...
  class EntityPost
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yext::EntityPost` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yext::EntityPost`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'entity_post_id')
        self.entity_post_id = attributes[:'entity_post_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          entity_post_id == o.entity_post_id &&
          entity == o.entity &&
    ...
    def hash
      [entity_post_id, entity, publisher, status, metrics, comments].hash
    end
