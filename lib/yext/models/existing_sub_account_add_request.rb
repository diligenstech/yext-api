...
module Yext
  class ExistingSubAccountAddRequest
    ...
    def self.openapi_types
      {
        ...
        :'sku_additions' => :'Array<SkuQuantity>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yext::ExistingSubAccountAddRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yext::ExistingSubAccountAddRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sku_additions')
        if (value = attributes[:'sku_additions']).is_a?(Array)
          self.sku_additions = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @sub_account_id.nil?
        invalid_properties.push('invalid value for "sub_account_id", sub_account_id cannot be nil.')
      end

      if @sku_additions.nil?
        invalid_properties.push('invalid value for "sku_additions", sku_additions cannot be nil.')
      end
    ...
    def valid?
      return false if @sub_account_id.nil?
      return false if @sku_additions.nil?
      true
    end
    ...
    def hash
      [sub_account_id, sku_additions, agreement_id].hash
    end
