...
module Yext
  ...
  class ListEntities200ResponseResponseEntitiesInnerLogo
    # Filtering Type: `text`
    ...
    def self.openapi_types
      {
        ...
        :'description' => :'String',
        :'details' => :'String',
        :'image' => :'ListEntities200ResponseResponseEntitiesInnerLogoImage'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yext::ListEntities200ResponseResponseEntitiesInnerLogo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yext::ListEntities200ResponseResponseEntitiesInnerLogo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
