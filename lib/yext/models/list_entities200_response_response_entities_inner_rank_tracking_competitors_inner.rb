...
module Yext
  ...
  class ListEntities200ResponseResponseEntitiesInnerRankTrackingCompetitorsInner
    # A name of a competitor   Cannot Include: * HTML markup  Filtering Type: `text`
    attr_accessor :name

    # The business website of a competitor   Cannot Include: * common domain names, e.g., google.com, youtube.com, etc.  Filtering Type: `text`
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yext::ListEntities200ResponseResponseEntitiesInnerRankTrackingCompetitorsInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yext::ListEntities200ResponseResponseEntitiesInnerRankTrackingCompetitorsInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @website.to_s.length < 0
      true
    end
