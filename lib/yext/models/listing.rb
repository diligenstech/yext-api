...
module Yext
  class Listing
    ...
    # *(only present if the **v** parameter is `20170420` or later)*  List of Publisher's alternate brands where the listing is syndicated 
    ...
    def self.openapi_types
      {
        ...
        :'alternate_brands' => :'Array<ListingAlternateBrandsInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yext::Listing` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yext::Listing`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, location_id, account_id, publisher_id, status, additional_status, listing_url, login_url, screenshot_url, status_details, alternate_brands].hash
    end
