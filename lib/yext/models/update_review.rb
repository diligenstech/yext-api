  ...
  class UpdateReview
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Yext::UpdateReview` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Yext::UpdateReview`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      flag_status_validator = EnumAttributeValidator.new('String', ["INAPPROPRIATE", "SPAM", "IRRELEVANT", "SENSITIVE", "NOT_FLAGGED"])
    ...
    def flag_status=(flag_status)
      validator = EnumAttributeValidator.new('String', ["INAPPROPRIATE", "SPAM", "IRRELEVANT", "SENSITIVE", "NOT_FLAGGED"])
    ...
    def hash
      [location_id, rating, content, author_name, author_email, status, flag_status].hash
    end
